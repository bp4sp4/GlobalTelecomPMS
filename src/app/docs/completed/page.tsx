  ...
  return (
    <AppShell brand={{ title: "문서관리", subtitle: "보고서/목록 관리" }} sections={docsNav()}>
      <div className={f.head}>
        <h1 className={f.title}>완료된 문서</h1>
        <p className={f.desc}>완료(DONE) 상태 문서만 집계합니다. 유형을 선택해 목록을 확인하세요.</p>
      </div>
      <div className={f.grid}>
        {TYPE_META.map((m) => (
          <Link key={m.type} href={`/docs/list?type=${m.type}&status=DONE`} className={f.card}>
            <span className={f.icon}>{m.icon}</span>
            <div className={f.body}>
              <h2 className={f.cardTitle}>{m.title}</h2>
              <p className={f.cardDesc}>완료본 · 열기</p>
            </div>
